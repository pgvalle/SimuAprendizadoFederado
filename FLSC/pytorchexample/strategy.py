...
class FLSC(FedAvg):
    ...
    def configure_evaluate(
        ...
        arrays_dict = self.unpack_arrays(arrays)
        np_arrays = {}
        weight = 1 / len(arrays_dict)
        for arrays in arrays_dict:
            for k, v in arrays.items():
                if k not in np_arrays:
                    np_arrays[k] = v.numpy() * weight
                else:
                    np_arrays[k] += v.numpy() * weight

        # load fusion as pytorch model
        arrays = ArrayRecord({k: Array(np.asarray(v)) for k, v in np_arrays.items()})
